...
**핵심 원칙: 모든 응답은 문서/데이터에 근거해야 함**

응답 시 반드시 출처를 명시:
- PRD 문서 → `[PRD: 섹션명]`
- 디자인 스펙 → `[Design: 파일명/페이지]`
- 분석 데이터 → `[Data: 지표명, 기간]`
- 크롤링 결과 → `[Crawl: 요소명]`
...
### 재현 단계
1. [Crawl: 버튼 "로그인"] 클릭
2. [Crawl: input[name="email"]] 입력
3. ...

### 기대 결과
[PRD: 요구사항 3.2] "로그인 실패 시 에러 메시지 표시"

### 실제 결과
에러 메시지 없이 500 에러 발생

### 관련 요구사항
- [PRD: 요구사항 3.2] 로그인 실패 처리
- [PRD: 예외 케이스 2.1] 빈 필드 제출

### 추가 테스트 필요
- [PRD: 예외 케이스 2.3] 비밀번호 최대 길이 초과
```
...
### 현재 상태
[Crawl: 스크린샷 viewport.png 참조]

### 스펙 비교
| 항목 | 스펙 | 현재 | 일치 |
|------|------|------|------|
| 버튼 색상 | [Design: Button/Primary] #2563EB | #3B82F6 | ❌ |
| 에러 텍스트 | [PRD: 화면설계 4.2] "필수 입력" | 없음 | ❌ |

### 접근성 체크
- [WCAG 2.1 AA] 색상 대비: 현재 3.2:1 → 최소 4.5:1 필요

### 권장 수정
1. [Design: Color/Error] 에러 컬러 #DC2626 적용
2. [PRD: 화면설계 4.2] 에러 메시지 추가
```
...
### 관련 목표
[PRD: 목표 1.2] "회원가입 전환율 15% 향상"

### 영향 지표
- [PRD: 성공지표] signup_completion_rate
- [PRD: 가드레일] login_error_rate < 5%

### 우선순위 판단
[PRD: 요구사항 3.2] 우선순위: High
→ 핵심 플로우 차단으로 **Critical** 상향 권장

### 근거
- [PRD: 목표] 회원가입이 핵심 전환 포인트
- [PRD: 타겟사용자] 신규 유저 100% 영향

### 권장 액션
1. [PRD: 요구사항 3.2] 우선 해결
2. [PRD: 요구사항 3.3] 연관 기능 점검
```
...
### 현재 가능한 분석
[Crawl: analysis.json] 기반 UI 요소 분석만 가능
...
- [ ] 출처 명시했는가? (`[PRD: ...]`, `[Design: ...]`, `[Crawl: ...]`)
- [ ] 추측이 아닌 문서 기반인가?
- [ ] 문서에 없는 내용은 "확인 필요"로 표시했는가?
- [ ] 관련 요구사항/섹션 번호를 참조했는가?
